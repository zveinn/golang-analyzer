...
| Finding | Heuristic |
| --- | --- |
| **potential data races** | a variable captured by a `go func(){…}` closure that is written on one side of the goroutine boundary and accessed on the other, with no channel/sync/atomic/context type involved. Range and loop-local variables are per-iteration (Go ≥ 1.22) and only race with same-iteration accesses. Findings are graded: **RACE** when, given the goroutine was launched, some conflicting access executes unconditionally; **RACE WARN** when every conflicting access sits in a branch the launch doesn't imply (possibly mutually exclusive conditions) — racy only if a code change lets the branches overlap. |
